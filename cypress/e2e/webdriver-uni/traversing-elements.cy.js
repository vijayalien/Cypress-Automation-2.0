...
describe("Traversing DOM elements in Cypress", () => {
  ...
  it("children() to get the children of DOM elements", () => {
    cy.get('.traversal-breadcrumb').scrollIntoView().children('.active').should('contain','Contact Us')
  });
  ...
  it("nextAll() to get all of the next sibling DOM elements within elements", () => {
    cy.get('.traversal-drinks-list').scrollIntoView().contains('Tea').nextAll().should('have.length',3)
  });

  it("nextUntil() to get all of the next sibling DOM elements within elements until another element", () => {
    cy.get('#coffee').nextAll('#espresso')
  });

  it("not() to remove DOM element(s) from the set of elements", () => {
    cy.get('.traversal-button-states > *').not('.disabled').should('not.have.class','disabled')
    cy.get('.traversal-button-states > *').not('.disabled').should('have.length','3')
  });
  ...
  it("parents() to get parents DOM element of elements", () => {
    cy.get('.traversal-cite').parents().should('match','div')
    cy.get('.traversal-cite').parents().should('have.class','traversal-cite-text')
  });

  it("prev() to get the previous sibling DOM element within elements", () => {
    cy.get('.traversal-food-list > *').contains('Figs').prev().contains('Cherries')
  });
  ...
  it.only("siblings() To get all sibling DOM elements of elements", () => {
    cy.get('.traversal-buttons').scrollIntoView().contains('Button').siblings().should('have.length',4)
  });
});
